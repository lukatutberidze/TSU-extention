...
function render() {
  ...
  for (let i = 0; i < table.children.length; i++) {
    ...
    for (let j = 1; j < table.children[i].children.length; j++) {
      ...
      for (let k = 0; k < table.children[i].children[j].children.length; k++) {
        ...
        if (
          table.children[i].children[j].children[k].className.includes("Equals")
        ) {
        ...
        try {
          for (let z = 0; z < 3; z++) {
            table_grandchildren.push(
              table.children[i].children[j].children[k].children[z].innerText
            );
          }
    ...
    Object.keys(maindict).forEach((key) => {
      ...
      for (let i = 0; i < value.length; i++) {
        for (let k = 0; k < value[i].length; k++) {
          if (
            value[i][k].length > 0 &&
            value[i][k][value[i][k].length - 1].includes(weekdays[j])
          ) {
            day.push([key, i, k]);
          }
        }
  ...
  function sortbytime(list) {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (dif > 1) {
        newls.splice(i + spliceint, 0, [
          `შესვენება ${dif - 1} საათი`,
          `${extractendtime(sorted[i - 1]) + 1}:00 - ${extracttime(
            sorted[i]
          )}:00`,
        ]);
        spliceint++;
